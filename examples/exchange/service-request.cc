...
class Client : public ONS_Service
{
  public:
    ...
    void		result(OTC_Time const& theTime);
};

Client::Client()
  : ONS_Service("","",OTCLIB_SERVICE_HIDDEN)
{
  subscribeStartup(&Client::startup);
}

void Client::startup()
{
  startTimer(&Client::call,1.0);
}

void Client::call()
{
  OTC_Iterator<OTC_ServiceBinding*> theServices;
  theServices = lookupServiceName("service");

  if (theServices.isValid())
  {
    OTC_String theConversationId;

    theConversationId = sendRequest(theServices.item(),
     "echo",OTC_Time::currentTime());

    processResponse(&Client::result,theConversationId);
  }

  startTimer(&Client::call,1.0);
}

void Client::result(OTC_Time const& theTime)
{
  cout << theTime << endl;
}
