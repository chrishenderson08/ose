...
import netrpc.soap
RemoteService = netrpc.soap.RemoteService
...
class RemoteServiceView:
  ...
  def __init__(self,root=None,**kw):
    ...
    if kw.has_key("ns"):
      namespace = kw["ns"]
    if kw.has_key("soapaction"):
      namespace = kw["soapaction"]
    ...
    self._serverField = Pmw.EntryField(f2,labelpos='w',entry_width=50,
        label_text="Service URL:",validate=None,value=server)
    self._serverField.pack(side="top",padx=4,pady=4)

    self._methodField = Pmw.EntryField(f2,labelpos='w',entry_width=50,
        label_text="Method Name:",validate=None,value=method)
    self._methodField.pack(side="top",padx=4,pady=4)

    self._namespaceField = Pmw.EntryField(f2,labelpos='w',entry_width=50,
        label_text="Method URI:",validate=None,value=namespace)
    self._namespaceField.pack(side="top",padx=4,pady=4)

    self._soapactionField = Pmw.EntryField(f2,labelpos='w',entry_width=50,
        label_text="SOAPAction:",validate=None,value=soapaction)
    self._soapactionField.pack(side="top",padx=4,pady=4)

    Pmw.alignlabels((self._serverField,self._methodField,
        self._namespaceField,self._soapactionField))
  ...
  def _executeMethod(self):
	...
	ns = self._namespaceField.get()
	if ns == "":
	  ns = None
	soapaction = self._soapactionField.get()
	if soapaction == "":
	  soapaction = None
	server = RemoteService(url,ns=ns,soapaction=soapaction)
