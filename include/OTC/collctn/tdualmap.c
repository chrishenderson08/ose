#ifndef OTC_COLLCTN_TDUALMAP_C
#define OTC_COLLCTN_TDUALMAP_C
/*
// ============================================================================
//
// = LIBRARY
...
OTC_TDualMap<T1,T2>::~OTC_TDualMap()
{
  count_->unReference();
  itemIndex_->unReference();
  keyIndex_->unReference();
  listByItems_->unReference();
  listByKeys_->unReference();
}
  ...
  : keyRankFn_(theKeyRankFn), itemRankFn_(theItemRankFn)
{
  ...
  listByKeys_ = new OTC_LinkList;
  OTCLIB_ASSERT_M(listByKeys_ != 0);
  listByKeys_->reference();

  listByItems_ = new OTC_LinkList;
  OTCLIB_ASSERT_M(listByItems_ != 0);
  listByItems_->reference();
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_TDualMap<T1,T2>::OTC_TDualMap(OTC_TDualMap<T1,T2> const& theMap)
  ...
  : keyRankFn_(theMap.keyRankFn_), itemRankFn_(theMap.itemRankFn_)
{
  ...
  listByKeys_ = theMap.listByKeys_;
  listByKeys_->reference();

  listByItems_ = theMap.listByItems_;
  listByItems_->reference();
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::removeAll()
{
  count_->set(0);
  itemIndex_->removeAll();
  keyIndex_->removeAll();
  listByKeys_->removeAll();
  listByItems_->removeAll();
}

/* ------------------------------------------------------------------------- */
template<class T1, class T2>
u_int OTC_TDualMap<T1,T2>::frequencyKey(T1 const& theKey) const
{
  OTC_AVLTreeNode* theIndexNode;
  theIndexNode = (OTC_AVLTreeNode*)keyIndex_->root();
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
u_int OTC_TDualMap<T1,T2>::frequencyItem(T2 const& theItem) const
{
  OTC_AVLTreeNode* theIndexNode;
  theIndexNode = (OTC_AVLTreeNode*)itemIndex_->root();

  if (searchIndexItem_(theIndexNode,theItem,theIndexNode) == 0)
    return theIndexNode->index()->population();
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
bool OTC_TDualMap<T1,T2>::containsKey(T1 const& theKey) const
{
  OTC_AVLTreeNode* theIndexNode;
  theIndexNode = (OTC_AVLTreeNode*)keyIndex_->root();
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
bool OTC_TDualMap<T1,T2>::containsItem(T2 const& theItem) const
{
  OTC_AVLTreeNode* theIndexNode;
  theIndexNode = (OTC_AVLTreeNode*)itemIndex_->root();

  return searchIndexItem_(theIndexNode,theItem,theIndexNode) == 0;
}

/* ------------------------------------------------------------------------- */
template<class T1, class T2>
bool OTC_TDualMap<T1,T2>::contains(
 T1 const& theKey,
 T2 const& theItem
) const
{
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_TDualMap<T1,T2>& OTC_TDualMap<T1,T2>::operator=(
 OTC_TDualMap<T1,T2> const& theMap
)
{
  if (&theMap != this)
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::add(T1 const& theKey, T2 const& theItem)
{
  int theRank;
  OTC_Link* theLink;
  OTC_AVLLinkNode* theLinkNode;
  ...
  if (theRank != 0)
  {
    ...
    if (theIndexNode == 0)
    {
      keyIndex_->addRoot(theNewIndexNode);
    }
    else if (theRank < 0)
    {
  ...
  OTCLIB_ENSURE_FN((theRank != 0),
   "OTC_TDualMap<T1,T2>::add(T1 const&, T2 const&)",
  ...
  // Now do it all again for the item map.

  theIndexNode = (OTC_AVLTreeNode*)itemIndex_->root();

  theRank = searchIndexItem_(theIndexNode,theItem,theIndexNode);
  ...
  if (theRank != 0)
  {
    ...
    if (theIndexNode == 0)
    {
      itemIndex_->addRoot(theNewIndexNode);
    }
    else if (theRank < 0)
    {
      theIndexNode->addBefore(theNewIndexNode);
  ...
  theRank = searchGroupKey_(theLinkNode,theKey,theLinkNode);
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::add(OTC_PairIterator<T1,T2> thePairs)
{
  while (thePairs.isValid())
  {
    add(thePairs.key(),thePairs.item());
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_PairIterator<T1,T2> OTC_TDualMap<T1,T2>::pairs(
...
) const
{
  ...
  theIter = new OTC_KeyItemCursor< T1,T2,OTC_PairBucket<T1,T2> >(
   listByKeys_,theDirection,theProtection);
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_Cursor<T1>* OTC_TDualMap<T1,T2>::keys_(
 T2 const& theItem,
...
) const
{
  ...
  theNode = (OTC_AVLTreeNode*)itemIndex_->root();

  theNode = rangeIndexItem_(theNode,theItem,theRankFn,theStartNode,theEndNode);
  ...
  theIter = new OTC_KeyCursor< T1,OTC_PairBucket<T1,T2> >(
   listByItems_,theStartLink,theEndLink,theDirection,theProtection);
  OTCLIB_ASSERT_M(theIter != 0);
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_Cursor<T2>* OTC_TDualMap<T1,T2>::items_(
 T1 const& theKey,
...
) const
{
  ...
  theNode = (OTC_AVLTreeNode*)keyIndex_->root();
  ...
  theIter = new OTC_ItemCursor< T2,OTC_PairBucket<T1,T2> >(
   listByKeys_,theStartLink,theEndLink,theDirection,theProtection);
  OTCLIB_ASSERT_M(theIter != 0);
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_PairCursor<T1,T2>* OTC_TDualMap<T1,T2>::pairsKey_(
 T1 const& theKey,
 int (*theRankFn)(T1 const&, T1 const&),
 OTC_Direction theDirection,
 OTC_Protection theProtection
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_PairCursor<T1,T2>* OTC_TDualMap<T1,T2>::pairsItem_(
 T2 const& theItem,
 int (*theRankFn)(T2 const&, T2 const&),
 OTC_Direction theDirection,
 OTC_Protection theProtection
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
T1 const& OTC_TDualMap<T1,T2>::pickKey() const
{
  OTCLIB_ENSURE_FN((population() != 0),
   "OTC_TDualMap<T1,T2>::pickKey()",
   "collection must not be empty");

  return ((OTC_PairBucket<T1,T2>*)listByKeys_->first())->key();
}

/* ------------------------------------------------------------------------- */
template<class T1, class T2>
T2 const& OTC_TDualMap<T1,T2>::pickItem() const
{
  OTCLIB_ENSURE_FN((population() != 0),
   "OTC_TDualMap<T1,T2>::pickItem()",
   "collection must not be empty");

  return ((OTC_PairBucket<T1,T2>*)listByItems_->first())->item();
}

/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::removeKey(
 T1 const& theKey,
 OTC_Protection theProtection
)
{
  // Trying to do this ourselves is a bit
  // complicated, so use an iterator to get
  // pairs and do one at a time.

  OTC_PairIterator<T1,T2> thePairs;
  thePairs = pairsKey(theKey);

  OTCLIB_ENSURE_FN((theProtection == OTCLIB_UNSAFE || thePairs.isValid()),
   "OTC_TDualMap<T1,T2>::removeKey(T1 const&, OTC_Protection)",
   "key doesn't exist");

  while (thePairs.isValid())
  {
    remove(thePairs.key(),thePairs.item());
    thePairs.next();
  }
}
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::removeItem(
 T2 const& theItem,
 OTC_Protection theProtection
)
{
  ...
  OTC_PairIterator<T1,T2> thePairs;
  thePairs = pairsItem(theItem);

  OTCLIB_ENSURE_FN((theProtection == OTCLIB_UNSAFE || thePairs.isValid()),
   "OTC_TDualMap<T1,T2>::removeItem(T2 const&, OTC_Protection)",
   "item doesn't exist");
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
void OTC_TDualMap<T1,T2>::remove(
 T1 const& theKey,
 T2 const& theItem,
 OTC_Protection theProtection
)
{
  ...
  OTCLIB_ENSURE_FN((theProtection == OTCLIB_UNSAFE || theRank == 0),
   "OTC_TDualMap<T1,T2>::remove(T1 const&, T2 const&, OTC_Protection)",
   "key doesn't exist");
  ...
  // Now do it all again for item map.
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::rankKey(T1 const& key1, T1 const& key2) const
{
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::rankItem(T2 const& item1, T2 const& item2) const
{
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_AVLTreeNode* OTC_TDualMap<T1,T2>::rangeIndexKey_(
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_AVLTreeNode* OTC_TDualMap<T1,T2>::rangeIndexItem_(
 OTC_AVLTreeNode* theRoot,
 T2 const& theItem,
 int (*theRankFn)(T2 const&, T2 const&),
 OTC_AVLTreeNode*& theStartNode,
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::searchIndexKey_(
 OTC_AVLTreeNode* theRoot,
 T1 const& theKey,
 OTC_AVLTreeNode*& theNode
) const
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::searchIndexItem_(
 OTC_AVLTreeNode* theRoot,
 T2 const& theItem,
 OTC_AVLTreeNode*& theNode
) const
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::searchGroupItem_(
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
int OTC_TDualMap<T1,T2>::searchGroupKey_(
 OTC_AVLLinkNode* theRoot,
 T1 const& theKey,
 OTC_AVLLinkNode*& theNode
) const
...
/* ------------------------------------------------------------------------- */
template<class T1, class T2>
OTC_Link* OTC_TDualMap<T1,T2>::link_(T1 const& theKey, T2 const& theItem)
{
