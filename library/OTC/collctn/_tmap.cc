...
void test10()
{
  ...
  map.add(pairs.pairs(),OTCLIB_REPLACE);
...
int test11_rank(char const* const& s1, char const* const& s2)
{
  int l1 = strlen(s1);
  int l2 = strlen(s2);
...
void test11()
{
  ...
  while (cin.good())
  {
    ...
    if (!cin.fail())
    {
      keys = map.keys((char const*)buf1,test11_rank);
      while (keys.isValid())
      {
        tracer() << keys.item() << endl;
        keys.next();
      }
      items = map.items((char const*)buf1,test11_rank);
      while (items.isValid())
      {
        tracer() << items.item() << endl;
        items.next();
      }
      pairs = map.pairsKey((char const*)buf1,test11_rank);
      while (pairs.isValid())
      {
